...
import { DEMO, getMatchById } from "@/lib/fixtures";
...
export async function POST(_req: Request, { params }: { params: { matchId: string } }) {
  ...
  // Mode démo : analyse ouverte, sans authentification ni quota.
  if (DEMO) {
    return NextResponse.json({
      ...
      prediction: predict(match.homeTeam, match.awayTeam),
      quota: { used: 0, limit: null, plan: "DEMO" },
    });
  }
